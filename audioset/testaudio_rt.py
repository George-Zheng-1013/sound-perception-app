        ...
        with open(CLASS_RISK_CSV, newline="", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                if r in ("safe", "s", "ok", "normal", "safe "):
                    ...
                    # 默认根据值长度或数值猜测
                    class_risk_map[cid] = (
                        "danger" if r in ("1", "true", "yes") else "safe"
                    )
...
def load_model(options):
    device = torch.device(
        "cuda" if options.get("use_gpu") and torch.cuda.is_available() else "cpu"
    )

    # 加载特征提取器
    checkpoint_path = r"Cnn14_16k_mAP=0.438.pth"
    ...
    # 加载合并权重
    combined_path = r"cnn14_mlp_combined.pt"
    if os.path.exists(combined_path):
        state = torch.load(combined_path, map_location=device, weights_only=False)
    ...
    while True:
        try:
            ...
            result = analyze_audio(
                global_model, global_classifier, None, global_labels, wav_path, {}
            )
            save_result_to_db(filename, result)
            print(
                f"[实时识别]: {result.get('class_name')} ({result.get('confidence'):.2%})"
            )
...
def init_db():
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS audio_results (id INTEGER PRIMARY KEY AUTOINCREMENT, audio_filename TEXT, class_name TEXT, class_id INTEGER, confidence REAL, distance REAL, risk_level TEXT, timestamp TEXT)"
    )
    conn.close()
...
def save_result_to_db(filename, result):
    ...
    conn.execute(
        "INSERT INTO audio_results (audio_filename, class_name, class_id, confidence, distance, risk_level, timestamp) VALUES (?, ?, ?, ?, ?, ?, ?)",
        (
            filename,
...
def api_latest_result():
    ...
    row = conn.execute(
        "SELECT * FROM audio_results ORDER BY id DESC LIMIT 1"
    ).fetchone()
    conn.close()
...
def api_analyze():
    ...
    if "audio" not in request.files:
        return (
            jsonify(
                {"status": "error", "message": "没有找到音频文件(需使用'audio'字段)"}
            ),
            400,
        )
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify({"status": "error", "message": "FFmpeg 转换失败，请检查环境"}),
                500,
            )
