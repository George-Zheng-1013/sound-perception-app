...
class CLAPWrapper():
    ...
    def __init__(self, model_fp: Union[Path, str, None] = None, version: str = '2023', use_cuda=False):
    ...
    def read_audio(self, audio_path, resample=True):
        ...
        audio_path_new = str(audio_path)
        sr = 32000
        audio_time_series, sample_rate = librosa.load(audio_path_new)
        resampled_waveform = librosa.resample(y=audio_time_series, orig_sr=sample_rate, target_sr=sr)
        audio_time_series = torch.from_numpy(resampled_waveform)
        
        resample_rate = self.args.sampling_rate
        if resample and resample_rate != sr:
    ...
    def get_embeddings(self, data):
        r"""Load preprocessed audio and return a audio embeddings"""
        with torch.no_grad():
            return self.clap.audio_encoder(data)
